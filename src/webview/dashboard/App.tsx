...
function Card({ title, children, style }: {
  ...
  return (
    <div style={{
      ...
      {title && (
        <p style={{ fontSize: '11px', fontWeight: 600, color: C.muted, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '12px', margin: '0 0 12px' }}>
          {title}
        </p>
      )}
      {children}
...
function ActivityHeatmap({ data }: { data: DailyActivity[] }) {
  ...
  return (
    <Card>
      ...
      <div style={{ display: 'flex', gap: '4px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', paddingRight: '4px', paddingTop: '0' }}>
            ...
            <div key={wi} style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
              {week.map((day, di) => {
                ...
                return (
                  <div
                    key={di}
                    title={`${format(day, 'MMM d, yyyy')}: ${count} messages`}
                    style={{
                      width:        '13px',
                      height:       '13px',
                      borderRadius: '3px',
...
function DashboardView({ stats, onOpenSession }: {
...
type State =
  ...
  | { status: 'loaded'; stats: DashboardStats };
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handler = (event: MessageEvent) => {
      const msg = event.data as { type: string; payload?: DashboardStats; message?: string };
      if (msg.type === 'STATS_DATA' && msg.payload) {
        setState({ status: 'loaded', stats: msg.payload });
      } else if (msg.type === 'STATS_ERROR') {
        setState({ status: 'error', message: msg.message || 'Failed to load stats.' });
      }
    };

    window.addEventListener('message', handler);
    vsc.postMessage({ type: 'REQUEST_STATS' });
    return () => window.removeEventListener('message', handler);
  }, []);

  const handleOpenSession = (sessionId: string) => {
    const vsc = acquireVsCodeApi();
    vsc.postMessage({ type: 'OPEN_SESSION', sessionId });
  };

  if (state.status === 'loading') { return <Loading />; }
  if (state.status === 'error')   { return <ErrorState message={state.message} />; }
  return <DashboardView stats={state.stats} onOpenSession={handleOpenSession} />;
}
